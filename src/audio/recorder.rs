...
pub(super) fn append_mono_f32(data: &[f32], channels: usize, out: &mut Vec<f32>) {
    if channels <= 1 {
...
pub(super) fn append_mono_i16(data: &[i16], channels: usize, out: &mut Vec<f32>) {
...
pub(super) fn append_mono_u16(data: &[u16], channels: usize, out: &mut Vec<f32>) {
    if channels <= 1 {
        out.extend(
            data.iter()
